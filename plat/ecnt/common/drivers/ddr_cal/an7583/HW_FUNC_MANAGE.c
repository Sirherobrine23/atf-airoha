/*
 * AN7583 HW_FUNC_MANAGE.c -- functional reconstruction from BL22 oracle.
 *
...
void DIG_HW_SHUF_IMPCAL_CFG(void *ctx, U32 channel, U32 shuffle)
{
    ...
    vPhyByteIO32WriteMsk(ctx, 0x03a01068U,
                          (U32)c[1] << 31, 0x80000000U);

    value = pack_imp_5bit_pair(c[3], c[4]);
    if (c[0] == 0U)
        value |= 0xc0000000U;
    vPhyByteIO32WriteMsk(ctx, 0x012010c8U, value, 0x3fffffffU);

    value = pack_imp_5bit_pair(c[3], c[4]);
    if (c[0] == 0U)
        value |= 0x80000000U;
    vPhyByteIO32WriteMsk(ctx, 0x012010ccU, value, 0x3fffffffU);
    ...
    vPhyByteIO32WriteMsk(ctx, 0x03a010c4U, value, 0xfffff1f7U);

    value = (((U32)c[7] << 24) & 0x7f000000U) |
            (((U32)c[8] << 16) & 0x007f0000U) |
            (((U32)c[9] <<  8) & 0x00007f00U) |
            (U32)c[10];
    vPhyByteIO32WriteMsk(ctx, 0x0ba00e90U, value, 0x7f7f7fffU);
    ...
    vPhyByteIO32WriteMsk(ctx, 0x03a0108cU, common, 0x70777777U);

    value = c[0] == 0U ? 0x00007777U : 0U;
    vPhyByteIO32WriteMsk(ctx, 0x03a01090U, value, 0x00007777U);
...
{
    vSetPHY2ChannelMapping(ctx, channel);
    vPhyByteIO32WriteMsk(ctx, 0x038006c4U,
...
void DIG_HW_NONSHUF_DQSG_SWITCH(void *ctx, U32 channel, U32 enable)
{
    ...
    vSetPHY2ChannelMapping(ctx, channel);
    vPhyByteIO32WriteMsk(ctx, 0x038006acU,
...
{
    vIO32WriteMsk_All(ctx, 0x03800110U, 0U, 0x20000000U);
    vIO32WriteMsk_All(ctx, 0x03800110U, 0U, 0x40000000U);
    vIO32WriteMsk_All(ctx, 0x0000017cU, 0U, 0U);
