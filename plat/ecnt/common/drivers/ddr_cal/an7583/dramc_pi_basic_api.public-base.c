...
extern void vPhyByteWriteFldAlign(void *ctx, U32 reg, U32 value, U32 field,
                                 U32 all_channels);
extern void EnableDramcPhyDCMNonShuffle(void *ctx, U32 enable);
...
{
    /* These are literal no-op writes in the AN7583 vendor object. */
...
{
    U32 rodten = raw_u32(ctx, 0x20);
    U32 inv = rodten == 0;
    U32 odt = 0;
    U8 rank_bak, rank;

    vPhyByteWriteFldAlign(ctx, 0x53a00f30U, inv, 0x00040116U, 0);
    vPhyByteWriteFldAlign(ctx, 0x5ba00fb0U, inv, 0x00040116U, 0);
    vIO32WriteMsk(ctx, 0x11000508U, 0x10000U, 0);
    vIO32WriteMsk(ctx, 0x19000588U, 0x10000U, 0x10000U);

    if (is_ddr4_family(ctx))
        odt = (rodten == 1U) ? 0x16U : 0x0eU;
    else if (is_ddr3_family(ctx))
        odt = 1U;
    ...
    for (rank = 0; (U32)rank < raw_u32(ctx, 0x08); rank++) {
        vSetRank(ctx, rank);
        vIO32WriteMsk(ctx, 0x91200eecU, odt, 0x3fU);
        vIO32WriteMsk(ctx, 0x99200f6cU, odt, 0x3fU);
    }
    vSetRank(ctx, rank_bak);

    vIO32WriteMsk(ctx, 0x012010a8U, rodten, 1);
    vIO32WriteMsk(ctx, 0x91200f04U, rodten << 15, 0x8000U);
    vIO32WriteMsk(ctx, 0x99200f84U, rodten << 15, 0x8000U);
    vIO32WriteMsk(ctx, 0xa3a01004U, rodten << 15, 0x8000U);

    vPhyByteWriteFldAlign(ctx, 0x51200f34U, inv, 0x00040100U, 0);
    vPhyByteWriteFldAlign(ctx, 0x99200fb4U, inv, 0x00040100U, 0);
    vPhyByteWriteFldAlign(ctx, 0x89200eb4U, inv, 0xff040100U, 0);
...
void EnableDramcPhyDCMShuffle(void *ctx, U32 enable)
{
    ...
    if (enable) {
        vPhyByteIO32WriteMsk_All(ctx, 0x0020168cU, 0, 0x0fU);
        ...
        vIO32WriteMsk_All(ctx, 0x012010a8U, 0, 2);
        ...
        vIO32WriteMsk_All(ctx, 0x002016acU, 0, 0x80000000U);
...
void EnableDramcPhyDCMNonShuffle(void *ctx, U32 enable)
{
    ...
    mode = *(const U32 *)(uintptr_t)(raw_u32(ctx, 0x90) + 4U);
    if (mode == 2U)
...
void DramcDRVinitSetting(void *ctx)
{
    ...
    vIO32WriteMsk_All(ctx, 0x090004bcU, 0x00001e00U, 0x00003f00U);
    vIO32WriteMsk_All(ctx, 0x090004bcU, 0x0000001eU, 0x0000003fU);
    vIO32WriteMsk_All(ctx, 0x090004bcU, 0x26000000U, 0x3f000000U);
    vIO32WriteMsk_All(ctx, 0x090004bcU, 0x00260000U, 0x003f0000U);

    vIO32WriteMsk_All(ctx, 0x090004c0U, 0x00002600U, 0x00003f00U);
    vIO32WriteMsk_All(ctx, 0x090004c0U, 0x00000026U, 0x0000003fU);
    vIO32WriteMsk_All(ctx, 0x090004c0U, 0x26000000U, 0x3f000000U);
    vIO32WriteMsk_All(ctx, 0x090004c0U, 0x00260000U, 0x003f0000U);

    vIO32WriteMsk_All(ctx, 0x090004c4U, 0x00002600U, 0x00003f00U);
    vIO32WriteMsk_All(ctx, 0x090004c4U, 0x00000026U, 0x0000003fU);

    vIO32WriteMsk_All(ctx, 0x1100053cU, 0x00001e00U, 0x00003f00U);
    vIO32WriteMsk_All(ctx, 0x1100053cU, 0x00000026U, 0x0000003fU);
    vIO32WriteMsk_All(ctx, 0x1100053cU, 0x1e000000U, 0x3f000000U);
    vIO32WriteMsk_All(ctx, 0x1100053cU, 0x00260000U, 0x003f0000U);

    vIO32WriteMsk_All(ctx, 0x11000544U, 0x00001e00U, 0x00003f00U);
    vIO32WriteMsk_All(ctx, 0x11000544U, 0x00000026U, 0x0000003fU);

    vIO32WriteMsk_All(ctx, 0x190005bcU, 0x00001e00U, 0x00003f00U);
    vIO32WriteMsk_All(ctx, 0x190005bcU, 0x00000026U, 0x0000003fU);
    vIO32WriteMsk_All(ctx, 0x190005bcU, 0x1e000000U, 0x3f000000U);
    vIO32WriteMsk_All(ctx, 0x190005bcU, 0x00260000U, 0x003f0000U);

    vIO32WriteMsk_All(ctx, 0x190005c4U, 0x00001e00U, 0x00003f00U);
    vIO32WriteMsk_All(ctx, 0x190005c4U, 0x00000026U, 0x0000003fU);

    vIO32WriteMsk_All(ctx, 0x012010d0U, 0x0000000dU, 0x0000001fU);
    vIO32WriteMsk_All(ctx, 0x012010d0U, 0x000001a0U, 0x000003e0U);
    vIO32WriteMsk_All(ctx, 0x012010d0U, 0x00003400U, 0x00007c00U);
    vIO32WriteMsk_All(ctx, 0x012010d0U, 0x00068000U, 0x000f8000U);
    vIO32WriteMsk_All(ctx, 0x012010d0U, 0x00d00000U, 0x01f00000U);
    vIO32WriteMsk_All(ctx, 0x012010d0U, 0x1a000000U, 0x3e000000U);

    vIO32WriteMsk_All(ctx, 0x012010d4U, 0x0000000dU, 0x0000001fU);
    vIO32WriteMsk_All(ctx, 0x012010d4U, 0x000001a0U, 0x000003e0U);
    vIO32WriteMsk_All(ctx, 0x012010d4U, 0x00003400U, 0x00007c00U);
    vIO32WriteMsk_All(ctx, 0x012010d4U, 0x00068000U, 0x000f8000U);
    vIO32WriteMsk_All(ctx, 0x012010d4U, 0x00d00000U, 0x01f00000U);
    vIO32WriteMsk_All(ctx, 0x012010d4U, 0x1a000000U, 0x3e000000U);
}
...
void vReplaceDVInit(void *ctx)
{
    ...
    /* Disable RX tracking/DVS for all ranks before applying DV defaults. */
    ...
    vIO32WriteMsk(ctx, 0x03800150U, 0, 0x00007800U);
    vIO32WriteMsk(ctx, 0x0000012cU, 0, 0x80000000U);

    broadcast = GetDramcBroadcast();
    DramcBroadcastOnOff(0);

    freq = raw_u16(ctx, 0x54);
    if (is_ddr3_family(ctx))
        cap_hyst = (freq <= 1065U) ? 3U : 0U;
    else if (is_ddr4_family(ctx))
        cap_hyst = 0U;
    else
        cap_hyst = 0x18U;

    val = (cap_hyst << 12) | 0x00400000U;
    vPhyByteIO32WriteMsk_All(ctx, 0x91200ef0U, val, 0x00c7f000U);
    vPhyByteIO32WriteMsk_All(ctx, 0x99200f70U, val, 0x00c7f000U);
    vPhyByteIO32WriteMsk_All(ctx, 0xa3a00ff0U, val, 0x00c7f000U);
    vPhyByteIO32WriteMsk_All(ctx, 0x89200e70U, val, 0x00c7f000U);

    val = (freq > 2132U) ? 0x00000c00U : 0U;
    vPhyByteIO32WriteMsk_All(ctx, 0x51200ef8U, val, 0x00000f00U);
    vPhyByteIO32WriteMsk_All(ctx, 0x59200f78U, val, 0x00000f00U);
    if (freq <= 300U)
        val |= 0x00000300U;
    vPhyByteIO32WriteMsk_All(ctx, 0x49200e78U, val, 0x00000f00U);

    vPhyByteWriteFldAlign(ctx, 0x01000808U, 0, 0x00ff0003U, 1);
    vPhyByteIO32WriteMsk_All(ctx, 0x01201120U, 0x00020000U, 0xff000001U);
    vIO32WriteMsk_All(ctx, 0x00000220U, 0, 0x00000040U);
    vIO32WriteMsk_All(ctx, 0x00000268U, 0, 0x00000040U);
    vIO32WriteMsk_All(ctx, 0x000002d8U, 0, 0x0000ffffU);
    ...
    vPhyByteIO32WriteMsk_All(ctx, 0x91200efcU, 0x00040012U, 2);
    vPhyByteIO32WriteMsk_All(ctx, 0x99200f7cU, 0x00040012U, 2);
