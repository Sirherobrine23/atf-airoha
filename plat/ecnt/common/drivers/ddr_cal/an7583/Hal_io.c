/*
 * AN7583 Hal_io.c -- functional reconstruction from BL22 Hal_io.o.
 *
...
static inline U32 virtual_type(U32 reg)
{
    return (reg >> 23) & 0xfU;
}
...
{
    volatile U32 *p;
    if (virtual_type(reg) == 7)
        return 0;
    p = (volatile U32 *)(uintptr_t)
...
void vPhyByteWriteFldAlign(void *ctx, U32 reg, U32 value, U32 field,
                           U32 all_channels)
{
    U32 start, end, shift, width, mask, encoded, ch;
    if (virtual_type(reg) == 7)
        return;
