/*
 * RX_path_auto_gen.c -- functional recovery in progress for AN7581.
 *
...
extern void PC3_DRAM_config(U32 data_rate, void *out);
extern void PC4_DRAM_config(U32 data_rate, void *out);
...
void get_RX_path_config(void *config_, U8 shuffle, U8 byte)
{
    ...
    U32 pc3_cfg_words[2];
    U32 pc4_cfg_words[2];
    U8 *pc3 = (U8 *)pc3_cfg_words;
    U8 *pc4 = (U8 *)pc4_cfg_words;
    ...
    (void)shuffle; /* BL22 AN7581 does not consume r1 after entry. */

    if (MEM_TYPE == 2U)
        PC3_DRAM_config(raw_u32(shu, 0), pc3_cfg_words);
    if (MEM_TYPE == 6U)
        PC4_DRAM_config(raw_u32(shu, 0), pc4_cfg_words);
...
static U32 rx_rank_pair(const U8 *rg, U32 base, U32 lo_idx, U32 hi_idx)
{
    U32 lo = raw_u32(rg, 4U * (base + lo_idx));
    U32 hi = raw_u32(rg, 4U * (base + hi_idx));
    return ((lo << 8) & 0xff00U) | (hi & 0x7fU);
}

static U32 rx_rank_triplet(const U8 *rg, U32 base, U32 a, U32 b, U32 c)
{
    U32 va = raw_u32(rg, 4U * (base + a));
    U32 vb = raw_u32(rg, 4U * (base + b));
    U32 vc = raw_u32(rg, 4U * (base + c));
    return (va << 24) | ((vb << 8) & 0xffffU) | (vc & 0x7fU);
}
...
void RX_PATH_auto_gen_and_set(void *ctx, U32 channel, U32 shuffle)
{
    ...
    v = ((raw_u32(rg, 0x5c) << 24) & 0x07000000U) |
        ...
        ((raw_u32(rg, 0x64) << 8) & 0x00000100U) |
        ((raw_u32(rg, 0x68) << 9) & 0x00000e00U) |
        ((raw_u32(rg, 0x6c) << 14) & 0x0000c000U) |
    ...
    v = ((raw_u32(rg, 0x94) << 16) & 0x001f0000U) |
        ...
        ((raw_u32(rg, 0x98) << 10) & 0x00007c00U) |
        ((raw_u32(rg, 0x9c) << 5) & 0x000003e0U);
    ...
    vPhyByteIO32WriteMsk(ctx, 0x012010a8U, v, 0xe00000f1U);

    for (rank = (U8)channel; config[0x04] > rank; rank++) {
        U32 base = (U32)config[0x04] * rank;
        ...
        vPhyByteIO32WriteMsk(ctx, 0x11600a24U,
                             rx_rank_pair(rg, base, 8U, 0x10U), 0xff7fU);
        vPhyByteIO32WriteMsk(ctx, 0x19600aa4U,
                             rx_rank_pair(rg, base, 9U, 0x11U), 0xff7fU);

        vPhyByteIO32WriteMsk(ctx, 0x11600a28U,
                             rx_rank_triplet(rg, base, 0x0cU, 8U, 0x10U),
                             0xff00ff7fU);
        vPhyByteIO32WriteMsk(ctx, 0x19600aa8U,
                             rx_rank_triplet(rg, base, 0x0dU, 9U, 0x11U),
                             0xff00ff7fU);
    }
    ...
    vSetRank(ctx, 1);
    v = ((raw_u32(rg, 0x150) << 7) & 0x80U) |
        (raw_u32(rg, 0x154) & 0x7fU) |
        ((raw_u32(rg, 0x158) << 15) & 0x8000U) |
        ((raw_u32(rg, 0x15c) << 8) & 0x7f00U);
    vPhyByteIO32WriteMsk(ctx, 0x01600b68U, v, 0xffffU);
}
