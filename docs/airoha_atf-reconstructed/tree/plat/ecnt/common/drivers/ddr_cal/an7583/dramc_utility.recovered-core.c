...
U16 DDRPhyGetRealFreq(void *ctx)
{
    ...
    if (meter == 0)
        return FreqByMPLLRG(raw_u8(ctx, 0x18));
...
void DramcEngine2CheckComplete(void *ctx, U8 status)
{
    ...
    while (timeout != 0) {
        ...
        if ((status & ~done) == 0) {
            if ((u4Dram_Register_Read(ctx, 0x108) & 0x8000U) != 0 &&
                vPhyByteReadFldAlign(ctx, 0x00800170U, 0) != 8U) {
                /* keep polling */
            } else if (shift_ui == 0) {
                return;
            } else if ((U16)u4Dram_Register_Read(ctx, 0x00800170U) == 2U) {
                return;
            }
        }
    ...
    if (uartDisable == 0)
        printf("TA2 Wait done bit timeout\\n");
}
...
static __attribute__((noinline)) void DramcBackupMixedRG_Single(void *ctx, const U32 *desc, U32 count)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        if (((field >> 18) & 3U) != 1U) {
            U32 shift = field & 0xffU;
            U32 mask = ((field >> 24) == 0xffU) ? 0U : mixed_mask(field);
...
void DramcSetRWOFOEN(void *ctx, U8 enable)
{
    ...
    if (timeout == 0 && uartDisable == 0)
        printf("RWOFOEN timout\n");
...
void DramcEngine2End(void *ctx)
{
    ...
    vPhyByteIO32WriteMsk(ctx, 0x108, 0x100, 0x100);
    vPhyByteIO32WriteMsk(ctx, 0x108, 0, 0x100);
    DramcSetRWOFOEN(ctx, 1);
    vPhyByteWriteFldAlign(ctx, 0x03800110, uiReg0D0h, 0, 0);
}
                     ...
                     U8 loop_count, U8 enable_ui_shift)
{
    ...
    uiReg0D0h = vPhyByteReadFldAlign(ctx, 0x03800110U, 0);
    vPhyByteIO32WriteMsk(ctx, 0x03800110U, 0, 0x00f000c0U);
    vPhyByteIO32WriteMsk(ctx, 0x108, 0, 0xe0000000U);
...
void TA2_Test_Run_Time_HW_Presetting(void *ctx, U32 len, U32 base, U32 rksel_mode)
{
    U32 old_channel = raw_u32(ctx, 0x04);
    ...
    vPhyByteIO32WriteMsk(ctx, 0x24c, 0xc0, 0xc0);
    offset = (rksel_mode == 3) ? (len >> 4) : (len >> 5);
    if (offset < 1)
        offset = 1;
    if (u4Dram_Register_Read(ctx, 0x10c) & 0x20000U)
        offset -= 2;

    mode_value = rksel_mode << 28;
    rwofoen = (rksel_mode == 3) ? 0 : 1;
    ...
    while ((U8)ch < raw_u32(ctx, 0x00)) {
        ...
        while ((U8)rk < raw_u32(ctx, 0x08)) {
            ...
            vIO32WriteMsk(ctx, 0x00400500U, base << 3, 0xfffffff8U);
        }
...
U32 TA2_Test_Run_Time_HW_Status(void *ctx)
{
    ...
    while ((U8)ch < raw_u32(ctx, 0x00)) {
        ...
        if (u4Dram_Register_Read(ctx, 0x100) & 0x40000U) {
            ...
            (void)vPhyByteReadFldAlign(ctx, 0x00800120U, 0);
        } else {
            (void)DramcEngine2Compare(ctx, 0);
        }
        bit_error |= vPhyByteReadFldAlign(ctx, 0x00800124U, 0);
        if (raw_u32(ctx, 0x44) == 8)
            bit_error = (U8)bit_error;
        vPhyByteIO32WriteMsk(ctx, 0x108, 0, 0xe0000000U);
    }
...
void TA2_Test_Run_Time_HW(void *ctx)
{
    ...
    TA2_Test_Run_Time_HW_Presetting(ctx, 0x10000, 0x400, 4);
    TA2_Test_Run_Time_Pat_Setting(ctx, 0);
    TA2_Test_Run_Time_HW_Write(ctx, 1);
    sum = TA2_Test_Run_Time_HW_Status(ctx);

    TA2_Test_Run_Time_HW_Presetting(ctx, 0x10, 0x800, 4);
    TA2_Test_Run_Time_HW_Write(ctx, 1);
    sum += TA2_Test_Run_Time_HW_Status(ctx);
    TA2_Test_Run_Time_HW_Presetting(ctx, 0x10, 0x1000, 4);
    TA2_Test_Run_Time_HW_Write(ctx, 1);
    sum += TA2_Test_Run_Time_HW_Status(ctx);
    TA2_Test_Run_Time_HW_Presetting(ctx, 0x10, 0x2000, 4);
    TA2_Test_Run_Time_HW_Write(ctx, 1);
    sum += TA2_Test_Run_Time_HW_Status(ctx);
    ...
    TA2_Test_Run_Time_HW_Presetting(ctx, 0x10, 0, 4);
    if (TA2_Test_Run_Time_HW_Status(ctx) == 0) {
        printf("SLT_BIN2\\n");
        printf("SLT_BIN2\\n");
        printf("SLT_BIN2\\n");
        for (;;)
    ...
    (void)DramcEngine2Init(ctx, raw_u32(ctx, 0x48), raw_u32(ctx, 0x4c),
                           raw_u8(ctx, 0x50), 0, 1);
