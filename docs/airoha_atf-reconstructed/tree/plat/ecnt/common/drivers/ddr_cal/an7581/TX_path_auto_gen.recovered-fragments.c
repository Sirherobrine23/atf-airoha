/*
 * TX_path_auto_gen.c -- functional recovery in progress for AN7581.
 *
...
void get_TX_path_config(void *config_, U8 shuffle, U8 byte)
{
    ...
    (void)shuffle; /* BL22 AN7581 object does not consume r1 after entry. */
    ...
    /* Initial values come from generated PC3/PC4 config and per-shuffle DV. */
...
/*
 * Recovered from AN7581 BL22 .text.TX_PATH_config.
 *
 * The generated RG block is rank-major for the fields below: rank 1 is the
 * same layout as rank 0 at +8 bytes.  The original code advances the base
 * pointer by eight bytes after programming rank 0.
 */
void TX_PATH_config(void *ctx, void *rg_, U32 channel, U32 byte)
{
    ...
    vPhyByteIO32WriteMsk(ctx, 0x002016e8U,
    ...
    for (rank = 0; rank < 2U; rank++) {
        ...
        value = (raw_u32(rg, ro + 0x70) & 0x1fffU) |
                ((raw_u32(rg, ro + 0x74) & 0x1fffU) << 16);
        ...
        value = tx_repeat_byte(raw_u32(rg, ro + 0x80));
        vPhyByteWriteFldAlign(ctx, 0x116009e0U, value, 0, 0);
        vPhyByteWriteFldAlign(ctx, 0x116009e4U, value, 0, 0);
        vPhyByteIO32WriteMsk(ctx, 0x116009ecU,
                             raw_u8(rg, ro + 0x80), 0xffU);

        value = tx_repeat_byte(raw_u32(rg, ro + 0x84));
        vPhyByteWriteFldAlign(ctx, 0x19600a60U, value, 0, 0);
        vPhyByteWriteFldAlign(ctx, 0x19600a64U, value, 0, 0);
        vPhyByteIO32WriteMsk(ctx, 0x19600a6cU,
                             raw_u8(rg, ro + 0x84), 0xffU);
        ...
        if (rank == 0U)
            ...
            value = ((U32)(rg[0xe6] & 0x1fU) << 8) |
                    (U32)(rg[0xe7] & 0x1fU);
        vPhyByteIO32WriteMsk(ctx, 0x00601234U, value, 0x1f1fU);

        d0 = raw_u32(rg, ro + 0xd0) & 0x3fU;
        d4 = raw_u32(rg, ro + 0xd4) & 0x3fU;
        if (top[0x76] != 0U) {
            if (top_flags != 0U) {
        ...
        if (pkg_type == 0U)
            value = f1 | 0x04000000U;
        else
            value = f1 | 0x13000000U;
        vPhyByteIO32WriteMsk(ctx, 0x096009a0U,
                             value, 0x3f3f3f00U);

        if (MEM_TYPE == 2U && top[0x76] == 0U) {
